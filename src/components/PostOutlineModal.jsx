...
const OutlineModal = ({ productId, onClose, selectedProductTitle }) => {
  ...
  // Fetch existing outline directly from the product document when modal opens
  useEffect(() => {
    const fetchProductOutline = async () => {
      try {
        ...
        if (productSnap.exists()) {
          ...
          // Load the outlines field if it exists
          setOutline(data.outlines || '');
        }
      } catch (error) {
        console.error('Error fetching product outline:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Update the 'outlines' field on that product document
      await updateDoc(productRef, {
        outlines: outline,
        updatedAt: new Date()
      });

      Swal.fire('Success', 'Outline saved successfully!', 'success');
      onClose();
    } catch (error) {
      console.error(error);
      Swal.fire('Error', 'Failed to save outline.', 'error');
    }
  };
    ...
    <ModalOverlay>
      <ModalContent>
        <h2 style={{ marginBottom: '1rem' }}>
          Edit Outline for {selectedProductTitle}
        </h2>

        {loading ? (
          <p style={{ textAlign: 'center', margin: '2rem 0' }}>Loading outline...</p>
        ) : (
          <>
            <Label>Outline Content (Separate by ENTER on new lines)</Label>
            <TextArea
              rows="6"
              placeholder="Write the course outline here..."
              value={outline}
