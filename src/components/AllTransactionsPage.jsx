...
const parseDateValue = (tx) => {
  const dateField = tx.paidAt || tx.createdAt;
  ...
  // Handle JavaScript Date objects or timestamp numbers
...
export default function AllTransactionsPage() {
  ...
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState("");
  const [sortAsc, setSortAsc] = useState(false); // false = Latest first, true = Earliest first
  ...
  const filtered = transactions
    .filter((tx) => {
      ...
      // Search matching by Name, Email, or Product Name
      return (
  ...
  return (
    <PageWrapper>
      ...
      <ControlsWrapper>
        ...
        <SortButton onClick={() => setSortAsc(!sortAsc)}>
          {sortAsc ? "Date: Earliest ▲" : "Date: Latest ▼"}
        </SortButton>
      </ControlsWrapper>
          ...
          {filtered.map((tx) => (
            <Card key={tx.id}>
              ...
              {tx.paidAt && (
                <Info>
                  🕓 Date:{" "}
                  {typeof tx.paidAt === "object" && tx.paidAt.seconds
                    ? new Date(tx.paidAt.seconds * 1000).toLocaleString()
                    : tx.paidAt}
                </Info>
              )}
            </Card>
